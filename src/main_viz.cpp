...
static void onMouseButton( GLFWwindow*, int i_btn, int i_act, int ) {
  ...
  if ( i_btn == GLFW_MOUSE_BUTTON_LEFT   ) g_mouseLeft   = ( i_act == GLFW_PRESS );
  if ( i_btn == GLFW_MOUSE_BUTTON_MIDDLE ) g_mouseMiddle = ( i_act == GLFW_PRESS );
}
static void onCursorPos( GLFWwindow*, double i_x, double i_y ) {
  float l_dx = (float)( i_x - g_lastX );
  float l_dy = (float)( i_y - g_lastY );
  g_lastX = i_x; g_lastY = i_y;
  if ( !g_camera ) return;
  if ( g_mouseLeft   ) g_camera->onMouseDrag(  l_dx, l_dy );
  if ( g_mouseMiddle ) g_camera->onMiddleDrag( l_dx, l_dy );
}
static void onScroll( GLFWwindow*, double, double i_dy ) {
  if ( ImGui::GetIO().WantCaptureMouse ) return;
  if ( g_camera ) g_camera->onScroll( (float)i_dy );
}
