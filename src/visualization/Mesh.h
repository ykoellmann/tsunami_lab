...
public:
  ...
  void init( t_idx i_nx, t_idx i_ny, float i_dx, float i_dy ) {
    ...
    for ( t_idx iy = 0; iy < i_ny - 1; iy++ )
      for ( t_idx ix = 0; ix < i_nx - 1; ix++ ) {
        ...
        l_idx.push_back( l_bl ); l_idx.push_back( l_br ); l_idx.push_back( l_tl );
        l_idx.push_back( l_br ); l_idx.push_back( l_tr ); l_idx.push_back( l_tl );
      }
    ...
    glBufferData( GL_ARRAY_BUFFER, (GLsizeiptr)( m_heights.size() * sizeof(float) ),
                  m_heights.data(), GL_DYNAMIC_DRAW );
    glVertexAttribPointer( 1, 1, GL_FLOAT, GL_FALSE, 0, nullptr );
    glEnableVertexAttribArray( 1 );

    glBindBuffer( GL_ELEMENT_ARRAY_BUFFER, m_ebo );
    glBufferData( GL_ELEMENT_ARRAY_BUFFER, (GLsizeiptr)( l_idx.size() * sizeof(unsigned int) ),
  ...
  void uploadHeights( const t_real* i_data, t_idx i_stride ) {
    for ( t_idx iy = 0; iy < m_ny; iy++ )
      for ( t_idx ix = 0; ix < m_nx; ix++ )
        m_heights[iy * m_nx + ix] = (float)i_data[ (iy + 1) * i_stride + (ix + 1) ];
  ...
  ~Mesh() {
    ...
    if ( m_vboH )   { glDeleteBuffers( 1, &m_vboH );   }
