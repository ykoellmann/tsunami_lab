...
void tsunami_lab::solvers::FWave::netUpdates(t_real i_hL,
  ...
  // compute wave speeds (Roe eigenvalues):  lambda_1 = u_Roe - sqrt(g * h_Roe); lambda_2 = u_Roe + sqrt(g * h_Roe)
  t_real l_waveSpeedL = 0;
  t_real l_waveSpeedR = 0;

  waveSpeeds(i_hL, i_hR, l_uL, l_uR, l_waveSpeedL, l_waveSpeedR);

  // compute wave strengths (eigencoefficients): [a_1, a_2]^T = R⁻¹ * delta f;  delta f = f(q_r) - f(q_l)
