...
void tsunami_lab::patches::WavePropagation2d::setGhost(BoundaryCondition i_left,
  ...
  for (t_idx l_iy = 0; l_iy < l_stride; l_iy++) {
    ...
    t_idx l_inner = getCoordinates(1, l_iy);
    l_h [l_ghost] = l_h [l_inner];
    l_hu[l_ghost] = (i_left == BoundaryCondition::Reflecting) ? -l_hu[l_inner] : l_hu[l_inner];
    l_hv[l_ghost] = l_hv[l_inner];
  ...
  for (t_idx l_iy = 0; l_iy < l_stride; l_iy++) {
    ...
    t_idx l_inner = getCoordinates(m_nCells_x,     l_iy);
    l_h [l_ghost] = l_h [l_inner];
    l_hu[l_ghost] = (i_right == BoundaryCondition::Reflecting) ? -l_hu[l_inner] : l_hu[l_inner];
    l_hv[l_ghost] = l_hv[l_inner];
    m_b[l_ghost]  = m_b[l_inner];
  }
  ...
  for (t_idx l_ix = 0; l_ix < m_nCells_x + 2; l_ix++) {
    ...
    l_h [l_ghost] = l_h [l_inner];
    l_hu[l_ghost] = l_hu[l_inner];
    l_hv[l_ghost] = (i_lower == BoundaryCondition::Reflecting) ? -l_hv[l_inner] : l_hv[l_inner];
    m_b[l_ghost]  = m_b[l_inner];
  }
  ...
  for (t_idx l_ix = 0; l_ix < m_nCells_x + 2; l_ix++) {
    ...
    l_h [l_ghost] = l_h [l_inner];
    l_hu[l_ghost] = l_hu[l_inner];
    l_hv[l_ghost] = (i_upper == BoundaryCondition::Reflecting) ? -l_hv[l_inner] : l_hv[l_inner];
    m_b[l_ghost]  = m_b[l_inner];
  }
}

void tsunami_lab::patches::WavePropagation2d::timeStep(t_real i_scaling, std::string) {
  t_idx l_stride = getStride();
