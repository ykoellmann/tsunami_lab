 ...
 * geographic location: the fault geometry comes from Wells & Coppersmith
 * scaling, the local depth/strike/dip from the Slab2 subduction model.
 **/
...
public:
   ...
   * the given location. The Slab2 model supplies the local fault depth, strike
   * and dip; Wells & Coppersmith scaling supplies slip, length and width.
   *
  ...
  fromMagnitudeAndLocation(double i_mw,
    ...
    WellsCoppersmith::FaultGeometry l_geo =
        WellsCoppersmith::fromMagnitude(i_mw);
