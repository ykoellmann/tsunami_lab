...
TEST_CASE("One-dimensional rare-rare setup", "[RareRare1d]") {
  tsunami_lab::setups::RareRare1d l_setup(25, 7, 3);
  ...
  SECTION("left side flows left (negative momentum)") {
    REQUIRE(l_setup.getMomentumX(2, 0) == Approx(-7));
  }

  SECTION("right side flows right (positive momentum)") {
  ...
  SECTION("momentum at the discontinuity belongs to the left side") {
    // spec says x <= x_dis → left state (which flows left in rare-rare)
