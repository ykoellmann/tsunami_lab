...
% ---------- Einstieg: 12h Rechnung, 4h Visualisierung, komisches Ergebnis ---
    \begin{frame}[plain]
        \centering
        \includegraphics[width=0.82\linewidth]{chile1000m_standbild_gut.png}\\[0.5cm]
        {\large 12 Stunden gerechnet. 4 Stunden visualisiert.}
    \end{frame}

    \begin{frame}[plain]
        \centering
        \includegraphics[width=0.82\linewidth]{chile1000m_standbild.png}\\[0.5cm]
        {\large Und dann kam \textcolor{coral}{das} dabei raus.}
    \end{frame}
...
% ---------- Der rote Faden ----------------------------------------------
    \begin{frame}{Die Reise eines Bebens}
        ...
        die Welle zum Bild, \textcolor{teal}{in Echtzeit}.}
    \end{frame}

% ======================================================================
% MIKA | Klick -> Quelle (~4 min)
% ======================================================================
    \divider{ston}{ston}{stoff}{stoff}{stoff}{Vom Klick zur Quelle}{Okada-Modell \& Parameterbestimmung}
...
% ---------- 1 Zahl -> 9 Parameter --------------------------------------
    ...
    \node[quelle, below=0.25cm of user] (slab) {\textbf{Slab2} (USGS)\\Tiefe, Streichen, Einfallen};
    \node[quelle, below=0.25cm of slab] (skal) {\textbf{Strasser 2010}\\Länge, Breite, Versatz};
    \node[annahme, below=0.25cm of skal] (fest) {\textbf{Feste Annahmen}\\Rake $90^\circ$, $\nu = 0{,}25$};
    ...
    \draw[pfeil] (fest.east) -- (okada.west|-fest.east) -- (okada);
        ...
        Außerhalb von Subduktionszonen greift der Fallback Wells \& Coppersmith 1994.}
    \end{frame}

% ---------- Strasser vs. Wells & Coppersmith ----------------------------
    \begin{frame}{Zwei Skalierungsgesetze, ein Erdbeben}
        \vspace{0.2cm}
        \begin{columns}[T]
            \begin{column}{0.48\textwidth}
                \raggedright
                {\color{coral}\bfseries\large Wells \& Coppersmith 1994}
                \vspace{0.35cm}

                \small
                {\color{coral}\textbullet}~Kalibriert auf krustale Beben (Blatt-, Auf-, Abschiebung)\\[0.22cm]
                {\color{coral}\textbullet}~Bis $M_w\!\sim\!8{,}1$ -- schlie{\ss}t Interface-Beben aus\\[0.22cm]
                {\color{coral}\textbullet}~Rolle: \textbf{Fallback} au{\ss}erhalb Slab2\\[0.5cm]
            \end{column}
            \begin{column}{0.48\textwidth}
                \raggedright
                {\color{teal}\bfseries\large Strasser et al.\ 2010}
                \vspace{0.35cm}

                \small
                {\color{teal}\textbullet}~Kalibriert speziell auf Subduktions-Interface-Beben\\[0.22cm]
                {\color{teal}\textbullet}~Slip momentenkonsistent statt regressiert: $D=M_0/(\mu LW)$\\[0.22cm]
                {\color{teal}\textbullet}~Rolle: genutzt \textbf{innerhalb} Slab2-Abdeckung
            \end{column}
        \end{columns}
        \vspace{0.5cm}
        \centering
...
% ---------- Okada-Feld ---------------------------------------------------
    ...
    % Hebungs-/Senkungsflaechen: exakt dieselbe Kontur wie die Bodenlinie,
    % per Clip farblich getrennt (kein Versatz an der Nahtstelle)
    \begin{scope}
        \clip (0,2.2) rectangle (11.2,3.9);
        \fill[amber!55, opacity=0.75]
        (0,2.2) -- (2.6,2.2)
        .. controls (3.6,2.2) and (3.4,2.95) .. (4.4,2.95)
        .. controls (5.4,2.95) and (5.2,1.75) .. (6.6,1.75)
        .. controls (7.6,1.75) and (7.6,2.2) .. (8.6,2.2) -- (11.2,2.2) -- cycle;
    \end{scope}
    \begin{scope}
        \clip (0,0.5) rectangle (11.2,2.2);
...
% ---------- Tanioka & Satake --------------------------------------------
    ...
    \node[teal, font=\small\bfseries, anchor=west] at (5.75,3.0) {wirkt wie Hebung};
        ...
        + u_y\,\tfrac{\partial b}{\partial y}\bigr)$}\\[0.15cm]
        {\small Tohoku \& Chile liegen an einer Tiefseerinne, deshalb eingebaut in
        \textcolor{teal}{Vorschau \& Live-Simulation} -- $u_x/u_y$ ebenso validiert
        gegen \texttt{okada85.m} ($\sim 5\cdot10^{-11}$).}
    \end{frame}
...
% ---------- Grenzen ------------------------------------------------------
    \begin{frame}{Ehrlich eingeordnet}
        ...
        dort kann kein Skalierungsgesetz präzise sein.}\\[0.3cm]

        \begin{columns}[T]
            \begin{column}{0.44\textwidth}
                \centering
                {\color{teal}\textbf{Chile ($M_w$ 8,8)}}\\[0.2cm]
                {\small trifft mit $\sim$469 vs.\ $\sim$450 km sehr genau.}
            \end{column}
            \begin{column}{0.44\textwidth}
                \centering
                {\color{coral}\textbf{{Tohoku} ($M_w$ 9,1, extremer Rand):}}\\[0.2cm]
                {\small Strasser überschätzt die Länge um $\sim$55\,\% (702 vs.\ $\sim$450 km)\\[0.1cm]}
            \end{column}
        \end{columns}
    \end{frame}
...
% ======================================================================
    \divider{stoff}{stoff}{ston}{ston}{stoff}{Von der Quelle aufs Display}{Live-Simulation \& Performance}
...
% ---------- Threads ------------------------------------------------------
    ...
    \node[block] (gui) {GUI / Render};
    \node[buf, right=1.7cm of gui] (buf) {SimBuffer\\{\footnotesize Double Buffer}};
    \node[block, right=1.7cm of buf] (solver) {SolverThread\\{\footnotesize adaptiver Zeitschritt}};
...
% ---------- Warum live geht: kein Datei-IO ------------------------------
    \begin{frame}{Früher: rechnen, schreiben, warten}
        \vspace{0.3cm}
        \centering
        {\small Gleiche Simulation, $10^6$ Zellen, 800 Zeitschritte, 10 Threads (M4),
        ...
        \begin{tikzpicture}[yscale=0.86]
            ...
            \node[foam, anchor=west, font=\small] at (0.1,3.30)
                {\textbf{Live-Pfad}: Frames in den Buffer, keine Dateien};
    ...
    \begin{frame}{Ein Kern zeigt, die anderen rechnen}
        ...
        \begin{tikzpicture}
            ...
            \node[teal, font=\footnotesize, align=center] at (7.4,-0.85)
                {\textbf{die übrigen Performance-Kerne},\\ein OpenMP-Thread pro Kern, ungestört};
        \end{tikzpicture}
        \\[0.6cm]
